...
public class DataExporter {
    ...
    public void addStepsCountToRun() {
        String query = "UPDATE run AS run " +
                "SET steps = s.steps " +
                "FROM (SELECT step.run_id, COUNT(*) AS steps FROM step AS step GROUP BY step.run_id) AS s " +
                "WHERE id = s.run_id";
    ...
    private void addStoredFunction() throws IOException {
        InputStream is = null;
        try {
            is = new FileInputStream("src/main/java/utwente/team2/databaseBackup/storedProcedure.sql");
        } catch (FileNotFoundException e) {
            e.printStackTrace();
        }
