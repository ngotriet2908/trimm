...
public enum StepDao {
    ...
    public Individual getIndividual(String variable, int runID) {
        try {
            String query = "SELECT MIN(s." + variable + ")" + ",MAX(s." + variable + ")" + ",AVG(s." + variable + ") FROM step s " +
                    "WHERE s.run_id = ? " +
                    "GROUP BY s.run_id";
            ...
            if (resultSet.next()) {
                return new Individual(resultSet.getBigDecimal(1).setScale(2, RoundingMode.HALF_UP).doubleValue(),
                        ...
                        resultSet.getBigDecimal(3).setScale(2, RoundingMode.HALF_UP).doubleValue(),variable);
            }
        } catch (SQLException sqle) {
            System.err.println("Error connecting: " + sqle);
        }
